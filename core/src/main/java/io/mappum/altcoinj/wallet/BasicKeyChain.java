...
public class BasicKeyChain implements EncryptableKeyChain {
     ...
     * @throws io.mappum.altcoinj.store.UnreadableWalletException.BadPassword if the password doesn't seem to match
     * @throws io.mappum.altcoinj.store.UnreadableWalletException if the data structures are corrupted/inconsistent
     */
    ...
    /**
     * Convenience wrapper around {@link #toEncrypted(io.mappum.altcoinj.crypto.KeyCrypter,
    ...
    public BasicKeyChain toEncrypted(KeyCrypter keyCrypter, KeyParameter aesKey) {
        ...
        try {
            ...
            for (ECKey key : hashToKeys.values()) {
                ...
                // (all bitcoinj controlled by that private key is lost forever).
