 ...
 *
 * <ul>
 * <li>{@code bitcoinj:<address>}</li>
 * <li>{@code bitcoinj:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://github.com/bitcoinj/bips/blob/master/bip-0021.mediawiki">BIP 0021</a>
 */
public class BitcoinURI {
    ...
    public static final String BITCOIN_SCHEME = "bitcoinj";
    ...
    public BitcoinURI(@Nullable NetworkParameters params, String input) throws BitcoinURIParseException {
        ...
        // URI is formed as  bitcoinj:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form bitcoinj://address?....
        // We support both until Ben fixes his code.
        
        // Remove the bitcoinj scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : bitcoinj:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("bitcoinj://")) {
            schemeSpecificPart = input.substring("bitcoinj://".length());
        } else if (input.startsWith("bitcoinj:")) {
            schemeSpecificPart = input.substring("bitcoinj:".length());
        } else {
        ...
        if (addressSplitTokens.length == 0)
            throw new BitcoinURIParseException("No data found after the bitcoinj: prefix");
