...
import io.mappum.altcoinj.script.Script.VerifyFlag;
import io.mappum.altcoinj.store.BlockStoreException;
import io.mappum.altcoinj.store.FullPrunedBlockStore;
    ...
    protected TransactionOutputChanges connectTransactions(int height, Block block)
        ...
        try {
            if (!params.isCheckpoint(height)) {
                ...
                // BIP30 document for more details on this: https://github.com/bitcoinj/bips/blob/master/bip-0030.mediawiki
            ...
            for (final Transaction tx : block.transactions) {
                ...
                if (valueOut.signum() < 0 || params.maxMoney != null && valueOut.compareTo(params.maxMoney) > 0)
                    throw new VerificationException("Transaction output value out of range");

                if (isCoinBase) {
                    coinbaseValue = valueOut;
                } else {
                    if (valueIn.compareTo(valueOut) < 0 || params.maxMoney != null && valueIn.compareTo(params.maxMoney) > 0)
            ...
            if ((params.maxMoney != null && totalFees.compareTo(params.maxMoney) > 0) || block.getBlockInflation(height).add(totalFees).compareTo(coinbaseValue) < 0)
    ...
    protected synchronized TransactionOutputChanges connectTransactions(StoredBlock newBlock)
        ...
        try {
            ...
            if (transactions != null) {
                ...
                if ((params.maxMoney != null && totalFees.compareTo(params.maxMoney) > 0) ||
