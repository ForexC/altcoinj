...
public class StoredPaymentChannelClientStates implements WalletExtension {
    ...
    @GuardedBy("lock") @VisibleForTesting final HashMultimap<Hash, StoredClientChannel> mapChannels = HashMultimap.create();
    ...
    public BigInteger getBalanceForServer(Hash id) {
     ...
     * channels found.
     */
    public long getSecondsUntilExpiry(Hash id) {
        lock.lock();
        try {
     ...
     */
    @Nullable
    StoredClientChannel getUsableChannelForServerID(Hash id) {
        lock.lock();
        try {
            Set<StoredClientChannel> setChannels = mapChannels.get(id);
            for (StoredClientChannel channel : setChannels) {
     ...
     */
    @Nullable
    StoredClientChannel getChannel(Hash id, Hash contractHash) {
        lock.lock();
        try {
            Set<StoredClientChannel> setChannels = mapChannels.get(id);
            for (StoredClientChannel channel : setChannels) {
    ...
    public byte[] serializeWalletExtension() {
        ...
        try {
            ...
            for (StoredClientChannel channel : mapChannels.values()) {
                ...
                checkState(channel.valueToMe.signum() >= 0 && (containingWallet.getParams().getMaxMoney() != null || channel.valueToMe.compareTo(containingWallet.getParams().getMaxMoney()) < 0));
                checkState(channel.refundFees.signum() >= 0 && (containingWallet.getParams().getMaxMoney() == null || channel.refundFees.compareTo(containingWallet.getParams().getMaxMoney()) < 0));
    ...
    public void deserializeWalletExtension(Wallet containingWallet, byte[] data) throws Exception {
        ...
        try {
            ...
            for (ClientState.StoredClientPaymentChannel storedState : states.getChannelsList()) {
                ...
                StoredClientChannel channel = new StoredClientChannel(new Hash(storedState.getId().toByteArray()),
                ...
                if (storedState.hasCloseTransactionHash())
                    channel.close = containingWallet.getTransaction(new Hash(storedState.toByteArray()));
...
class StoredClientChannel {
    ...
    StoredClientChannel(Hash id, Transaction contract, Transaction refund, ECKey myKey, BigInteger valueToMe,
